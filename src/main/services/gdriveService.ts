...
export class GoogleDriveService {
  ...
  static async refreshAccessToken(
    ...
    info('[Sync] Refreshing Google OAuth access token...')
    ...
    if (!res.ok) {
      ...
      error('[Sync] Failed to refresh token:', errText)
  ...
  static startOAuthFlow(
    ...
    info('[Sync] Starting Google OAuth local listener server...')
    ...
    server = http.createServer(async (req, res) => {
      ...
      const err = parsedUrl.query.error as string | undefined
      ...
      try {
        ...
        if (!tokenData.refresh_token) {
          throw new Error(
            'No refresh token returned. Try removing BrickForge permissions from your Google Account settings and re-connecting.'
          )
        }
        ...
        res.end(`
          ...
          <html>
          <head>
            <title>BrickForge Authorized</title>
            <style>
              body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; text-align: center; padding: 50px; background: #1e1e1e; color: #cccccc; }
              .card { max-width: 500px; margin: auto; padding: 30px; border-radius: 6px; background: #2d2d2d; border: 1px solid #3c3c3c; box-shadow: 0 4px 20px rgba(0,0,0,0.5); }
              h1 { color: #10b981; margin-top: 0; }
              p { color: #969696; line-height: 1.5; }
              .email { font-weight: bold; color: #007acc; }
            </style>
          </head>
          <body>
            <div class="card">
              <h1>Successfully Connected!</h1>
              <p>BrickForge has been linked to <span class="email">${emailData.email}</span>.</p>
        ...
        error('[Sync] OAuth token exchange error:', ex)
        res.writeHead(500, { 'Content-Type': 'text/html' })
        res.end(`<h1>Authorization Error</h1><p>${ex.message}</p>`)
    ...
    server.listen(this.PORT, '127.0.0.1', () => {
      info(`[Sync] OAuth local server listening on 127.0.0.1:${this.PORT}`)
    })
  ...
  static async findDatabaseFile(
    accessToken: string,
    dbName: string = 'brickforge.db'
  ): Promise<DriveFile | null> {
    info(`[Sync] Searching for ${dbName} in Google Drive appDataFolder...`)
    const searchUrl =
    ...
    if (!res.ok) {
      const errText = await res.text()
      error('[Sync] Find database file failed:', errText)
  ...
  static async listDatabaseFiles(accessToken: string): Promise<DriveFile[]> {
    info('[Sync] Listing database files in Google Drive appDataFolder...')
    const searchUrl =
      'https://www.googleapis.com/drive/v3/files?' +
    ...
    if (!res.ok) {
      const errText = await res.text()
      error('[Sync] List database files failed:', errText)
  ...
  static async uploadDatabaseFile(
    ...
    info(`[Sync] Creating database file ${dbName} in Google Drive appDataFolder...`)
    ...
    const boundary = 'brickforge_multipart_boundary'
    const delimiter = `\r\n--${boundary}\r\n`
    ...
    if (!res.ok) {
      const errText = await res.text()
      error('[Sync] Upload database file failed:', errText)
      throw new Error(`Drive upload failed: ${res.statusText} (${errText})`)
    }

    const data = (await res.json()) as DriveFile
    info(`[Sync] Database file ${dbName} uploaded successfully. ID: ${data.id}`)
    return data
  }
  ...
  static async updateDatabaseFile(
    ...
    info(`[Sync] Updating database file ${fileId} in Google Drive...`)
    const fileBuffer = await fs.readFile(localFilePath)

    const res = await fetch(
      `https://www.googleapis.com/upload/drive/v3/files/${fileId}?uploadType=media&fields=id,name,mimeType,modifiedTime,md5Checksum`,
      {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${accessToken}`,
    ...
    if (!res.ok) {
      const errText = await res.text()
      error('[Sync] Update database file failed:', errText)
      throw new Error(`Drive patch failed: ${res.statusText} (${errText})`)
    }

    const data = (await res.json()) as DriveFile
    info(`[Sync] Database file updated successfully. MD5: ${data.md5Checksum}`)
    return data
  }
  ...
  static async downloadDatabaseFile(
    ...
    info(`[Sync] Downloading database file ${fileId} to ${destinationPath}...`)
